...
## ۴. استقرار آزمایشی روی Vercel

این مسیر فقط برای **دمو و نمایش به مشتری** است، نه محیط واقعی. دلیلش در انتهای
همین بخش آمده.

### قدم‌ها

**۱. دیتابیس بگیرید.** Vercel دیتابیس میزبانی نمی‌کند. یکی از این‌ها:
[Neon](https://neon.tech)، [Supabase](https://supabase.com) یا Vercel Postgres.
همه پلن رایگان دارند.

رشته اتصالِ **pooled** را بردارید، نه مستقیم. در حالت serverless هر درخواست
می‌تواند یک اتصال تازه باز کند و دیتابیس زود اشباع می‌شود:

```
postgresql://user:pass@host/db?sslmode=require&pgbouncer=true&connection_limit=1
```

**۲. پروژه را به Vercel وصل کنید** (از داشبورد، با مخزن گیت‌هاب).

**۳. متغیرهای محیطی را در تنظیمات پروژه بگذارید:**

```
DATABASE_URL=postgresql://...          # رشته pooled
...
**۴. Deploy کنید.** فایل `vercel.json` دستور build را روی `npm run vercel-build`
گذاشته که قبل از build مهاجرت‌ها را اجرا می‌کند.

**۵. کاتالوگ اولیه را یک‌بار وارد کنید** — از روی سیستم خودتان:

```bash
DATABASE_URL="<همان رشته اتصال>" npx tsx prisma/seed.ts
```

### چه چیزی روی Vercel کار نمی‌کند

| قابلیت | وضعیت |
|---|---|
| سایت، پنل، سفارش، ترب، پیامک | کار می‌کند |
| **آپلود تصویر محصول و فیش واریزی** | **کار نمی‌کند** مگر با S3 |
| پشتیبان‌گیری خودکار | کار نمی‌کند (سرویس داکر است) |
| محدودیت نرخ | ضعیف می‌شود |

**آپلود** مهم‌ترینشان است: فایل‌سیستم Vercel فقط‌خواندنی است. کد این را تشخیص
می‌دهد و پیام روشن نشان می‌دهد به‌جای خطای گنگ. برای فعال کردنش یک فضای ابری
S3-سازگار بگیرید و متغیرهای `S3_*` را پر کنید.

**محدودیت نرخ** در حافظه هر لامبدا شمرده می‌شود و لامبداها مستقل‌اند، پس سقف
واقعی چند برابر می‌شود. برای دمو مهم نیست.

**پشتیبان‌گیری خودکار** سرویس داکر است و روی Vercel اجرا نمی‌شود؛ برای دمو هم
لازم نیست.

### هشدار مهم برای مخاطب ایرانی

Vercel از ایران معمولاً بدون تحریم‌شکن باز نمی‌شود و ثبت‌نام و استقرار هم
ممکن است با IP ایران مسدود شود. اگر مشتری شما در ایران است، احتمال زیادی
دارد که لینک `vercel.app` برایش باز نشود.

برای دموی مخاطب ایرانی، **لیارا گزینه بهتری است**: از داخل ایران باز می‌شود،
دیسک دائمی می‌دهد (پس آپلود هم کار می‌کند) و همان `Dockerfile` این پروژه را
اجرا می‌کند. راهنمایش در بخش قبل است.
