...
/*
  ...
  روی زمینه تیره هیرو، این لوگوها با فیلتر `brightness-0 invert` سفید می‌شوند.
  آن فیلتر هر پیکسل مات را سفید می‌کند و فرقی بین جوهر لوگو و پس‌زمینه‌اش
  نمی‌گذارد؛ پس فایل باید *فقط جوهر* داشته باشد و بقیه‌اش شفاف باشد.

  نجمیه و بقیةالله پس‌زمینه سفید مات داشتند (۶۲٪ و ۲۰٪ از پیکسل‌های دیده‌شده)
  و به لکه سفید بی‌شکل تبدیل می‌شدند. نسخه `-mono` از رویشان ساخته شد که در آن
  آلفای هر پیکسل بر اساس تیرگی‌اش بازنویسی شده. اصل فایل‌ها کنارشان مانده است.

  اگر لوگوی تازه‌ای اضافه کردید و اینجا لکه سفید دیدید، علتش همین است:
